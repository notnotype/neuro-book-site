...
// 注册页：全屏居中卡片，不套默认布局。后端强制邀请码制，必须提交 inviteCode。
...
async function submit(): Promise<void> {
    ...
    try {
        await $fetch("/api/auth/register", {
            method: "POST",
            body: {username: username.value, password: password.value, inviteCode: inviteCode.value.trim()},
        });
        await refresh();
...
<template>
    <main class="flex min-h-screen items-center justify-center bg-[var(--bg-main)] p-6 text-[var(--text-main)]">
        <Panel as="form" class="w-full max-w-sm space-y-4" @submit.prevent="submit">
            ...
            <!-- GitHub OAuth 入口：未绑定账号会进补全注册（同样需要邀请码，免设密码） -->
            ...
            <a v-if="githubOAuthEnabled" href="/auth/github" class="block"><Button type="button" variant="secondary" block><span class="i-lucide-github h-4 w-4"></span>使用 GitHub 注册</Button></a>
