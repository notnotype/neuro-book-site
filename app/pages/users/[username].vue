...
<template>
    <StateBlock v-if="notFound" state="empty" message="用户不存在" />
    ...
    <section v-else-if="user" class="flex flex-col gap-6">
        ...
        <Panel class="flex items-start gap-4">
            ...
            <div class="min-w-0 flex-1">
                ...
                <p class="text-sm text-[var(--text-muted)]">@{{ user.username }} · 加入于 {{ formatDate(user.joinedAt) }}</p>
        ...
        <!-- 该作者的公开条目 -->
        <div>
            <h2 class="mb-3 text-sm font-medium text-[var(--text-secondary)]">公开条目（{{ user.items.length }}）</h2>
            <StateBlock v-if="user.items.length === 0" state="empty" message="该作者还没有公开条目" />
