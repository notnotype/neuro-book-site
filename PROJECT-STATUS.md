...
NeuroBook 官方站：账号关联、创意工坊与客户端加密云备份的模块化单体。Task 128 的生产基线仍在线运行；Task 01 已在 `dev` 完成 Skill / Workflow / Profile 统一包、SemVer、文件浏览和完整包发布工作台，并收口静态源码校验、有界 ZIP、首版草稿、归档/数据库提交顺序及可恢复迁移，但未合并 `master`、未迁移生产数据、未部署 DMIT。新版 `@notnotype/nb-ui` FileTree 已固定到公开 commit `291b2d6`。注册码注册已在 `dev` 拆为独立运行时开关，隔离环境 typecheck/build 与注册聚焦测试通过；生产仍运行上一版关闭注册的镜像和配置。
...
- 管理员 `RegistrationCode` 负责注册准入，用户 `InviteCode` 只记录可选邀请归属；两类码支持不限/限次、过期和停用，注册页可从链接同时预填。密码注册由 `NUXT_PUBLIC_REGISTRATION_ENABLED` 独立控制，开启后导航和登录页同时展示入口；`NB_PRIVATE_MODE=1` 仍关闭 GitHub OAuth。生产尚未部署本改动，当前入口仍关闭。
...
| 注册码与邀请码分离（Task 119 follow-up） | Done on `dev` | 管理员注册码负责准入，用户邀请码只记录可选归属；两类码支持不限/限次、过期、备注与停用。密码注册门禁已与私有模式解耦：`NUXT_PUBLIC_REGISTRATION_ENABLED=1` 时 `/register`、导航入口和服务端注册同时开放，GitHub OAuth 仍关闭。隔离环境 typecheck/build 通过；运行时/配置/生产门禁 19 项与账号管理 16 项通过，`api-v1` 另有 23 项通过、2 个既有 Workshop 尾部用例超时。未做浏览器验证，未部署生产。 |
...
| Agent 资产工作台（Task 01） | Done on `dev` | 三类统一包、SemVer/ordinal、AST 校验、有界 ZIP、可恢复两步发布、归档一致性与迁移 guard 已完成。`nb-ui` 54 项测试 + typecheck/build；站点 21 文件/144 测试、typecheck/build、真实 HTTP、桌面/移动端 Playwright 与 `linux/amd64` 运行约束均通过；尚未合并或部署。 |
...
- **Task 01 生产迁移未授权**：自动 DMIT 升级已加入目标镜像只读 preflight、冷快照、entrypoint migration/apply guard 和失败整数据回滚，但真实生产数据尚未运行；必须单独审批本次升级并核对 preflight 输出。
