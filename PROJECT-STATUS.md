...
NeuroBook 官方站：账号关联、创意工坊与客户端加密云备份的模块化单体。Task 128 的代码生产化已完成：仓库硬切为 `neuro-book-site`，Workshop 有界流式上传、跨 Workshop/Backup 全站容量门禁、密文 `.nbbackup`、私有模式、live/ready、stdin 管理员初始化和非 root 只读容器合同均已落地。Windows typecheck/build 与 121 项测试通过；`arch` 干净 `linux/amd64` 镜像、空卷 migration、Secure Cookie、真实 Bearer Workshop/Backup 上传、容器重建持久性已验证。公开仓库/GHCR、DMIT loopback、DNS/证书/443 和真实 NeuroBook 公网闭环仍待执行；公开邀请 Gate 不在本任务内。
...
- 生产 owner-only 私有模式服务端关闭注册和 GitHub OAuth，前端隐藏入口；开发模式仍保留邀请码注册与 OAuth 代码供后续独立 Gate 使用。
...
| 官方站生产化与部署（Task 128） | In Progress | 仓库/品牌/依赖硬切、密文 Backup、全站容量、Workshop 有界流式解析、私有模式、生产配置门禁、live/ready、stdin 初始化、Docker/Compose/Actions 与运维文档已完成。121 项测试和 `arch` 容器持久化 smoke 通过；待公开 GitHub/GHCR、DMIT loopback、维护窗口 443 与真实客户端闭环。 |
...
- **Task 128 公网阶段待完成**：公开仓库/GHCR digest 尚未创建，DMIT 尚未安装 Docker，DNS/证书/443 未修改。HTTPS 稳定前不能把 NeuroBook 默认官方站地址改为 `https://nbook.notnotype.com`。
