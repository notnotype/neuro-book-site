...
| 官方站 OAuth provider 与 llmlint SSO（Task 133） | In Progress | provider、固定 `llmlint-web` client、llmlint S256 PKCE 回调和 DMIT 公网切换均已完成；PR #2 的顶层表单 POST 已部署并让真实浏览器到达 callback。后续 token 401 已定位为 OAuth library 未逆解码标准 Basic 表单编码，本分支已在 transport 层规范化并用含保留字符 secret 完成红绿回归，待合入部署后复验 callback 与 20 份盲评。 |
