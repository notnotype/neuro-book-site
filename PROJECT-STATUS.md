...
NeuroBook 官方站：账号关联、创意工坊与客户端加密云备份的模块化单体。Task 01 的 Skill / Workflow / Profile 统一包、SemVer、文件浏览、完整包发布工作台、静态源码校验、有界 ZIP、首版草稿和可恢复迁移已部署 DMIT。新版 `@notnotype/nb-ui` FileTree 固定到公开 commit `291b2d6`。密码注册已独立开放，GitHub OAuth 继续关闭；Task 02 的账号显示名称、中英双语、认证表单与开关链路收口均已部署并完成公网验收。t133 provider 与 llmlint SSO 已切换到 DMIT 公网，consent navigation 与标准 Basic 表单解码修复均已部署，隔离真实浏览器账号已完成授权、token/userinfo、本地映射和盲评池读取闭环；待 owner admin 完成 20 份双轴盲评。
...
- 登录与注册 mutation 直接应用接口返回的权威 session；只有首次打开带顶栏的页面才读取 `/api/auth/me`，避免账号已经创建后被第二次请求误报为失败。
...
- 第一方 OAuth provider 固定 `llmlint-web`、`code` + S256 PKCE 与 `client_secret_basic`；授权确认页使用顶层表单导航跟随 302，批准端点只接受精确 Origin 和严格的 JSON/URL 编码批准体。token transport 会先逆转 RFC 6749 对 Basic client ID/secret 的表单编码，再交给当前固定版本 OAuth library 校验。
...
| 官方站 OAuth provider 与 llmlint SSO（Task 133） | In Progress | provider、固定 `llmlint-web` client、llmlint S256 PKCE 回调和 DMIT 公网切换均已完成；PR #2 修复 consent 顶层导航，PR #3 修复标准 Basic 表单解码，两版均已部署。隔离真实浏览器账号已验证 callback、token/userinfo、本地 `neuroBookUserId` 映射和 20 项盲评池读取，临时数据已清理；待 owner admin 提交 20 份双轴盲评。 |
